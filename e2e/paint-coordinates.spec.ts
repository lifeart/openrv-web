...
test.describe('Paint Coordinate System', () => {
  test('should correctly map High Aspect Ratio coordinates (Unit Height assumption)', async ({ page, appPage }) => {
    ...
    builder.object('source', 'RVFileSource', 1).component('media').string('movie', testFileName).end().end();
    ...
    // Frame order
    paintObject.component('frame:1').string('order', ['pen:pen_test:1:user', 'text:text_test:1:user']).end();
