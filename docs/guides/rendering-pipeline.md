...
Production renderer construction (`src/ui/components/ViewerGLRenderer.ts`, `src/workers/renderWorker.worker.ts`) now routes through `createRenderer(caps)` (commits MED-55 P-pre-1/-2). The factory in `src/render/createRenderer.ts` selects `WebGPUBackend` when `caps.webgpuAvailable && caps.webgpuHDR`, otherwise falls back to the WebGL2 `Renderer`. The WebGPU stage pipeline is gated by a tristate feature flag (default disabled) — see `src/render/webgpu/featureFlag.ts`. Phase 4a stage WGSL registration into `WebGPUShaderPipeline.registerStage` is wired but defaults to `'disabled'` mode in production. The per-stage `@vertex fn vs` declarations have already been removed from the 11 stage WGSL files (commit `6444da9`); the orchestrator prepends the shared `_viewer_vert.wgsl` (first stage) or `_passthrough_vert.wgsl` (intermediate stages) at compile time.
