...
export class PaintEngine extends EventEmitter<PaintEngineEvents> {
  ...
  private _sourceIndex: number | undefined = undefined;
  ...
  /**
   * Get the source index stamped on new annotations.
   * When set, new annotations record the actual media source they were drawn on,
   * so they follow the source rather than the A/B slot when sources are reassigned.
   */
  get sourceIndex(): number | undefined {
    return this._sourceIndex;
  }

  set sourceIndex(value: number | undefined) {
    ...
    for (const [_annotationFrame, annotations] of this.state.annotations) {
      for (const annotation of annotations) {
        if (
          ...
          this.matchesSourceOrVersionFilter(annotation, versionFilter, sourceIndexFilter) &&
  ...
  getAnnotationsWithGhost(
    ...
    for (const [annotationFrame, annotations] of this.state.annotations) {
      for (const annotation of annotations) {
        if (!this.matchesSourceOrVersionFilter(annotation, versionFilter, sourceIndexFilter)) continue;
  ...
  /**
   * Match annotations by source index when available, falling back to version tag
   * for legacy annotations without sourceIndex. Annotations with version 'all' or
   * no version are always visible regardless of source index.
   */
  private matchesSourceOrVersionFilter(
    annotation: Annotation,
    versionFilter?: 'A' | 'B',
    sourceIndexFilter?: number,
  ): boolean {
    // No filter applied — show everything
    if (!versionFilter && sourceIndexFilter === undefined) return true;
    ...
    // If the annotation has a sourceIndex and a sourceIndexFilter is provided,
    // match by source identity (annotation follows the source, not the slot)
    if (annotation.sourceIndex !== undefined && sourceIndexFilter !== undefined) {
      return annotation.sourceIndex === sourceIndexFilter;
    }

    // Fall back to version-based filtering for legacy annotations without sourceIndex
    if (!versionFilter) return true;
    return v === versionFilter;
  }
