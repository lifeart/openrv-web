 ...
 * Video (movie/proxy) representations cannot round-trip through serialization
...
const mockCreateObjectURL = vi.fn().mockReturnValue('blob:http://localhost/mock-video-blob-url');
vi.stubGlobal('URL', {
  ...URL,
...
describe('VideoRepresentationLoader (issue #530)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should load from file when file is provided', async () => {
    ...
    const rep = makeRepresentation({ file, fps: 24 });
  ...
  it('should populate url on loaderConfig after file-based load', async () => {
    ...
    expect(rep.loaderConfig.url).toBe('blob:http://localhost/mock-video-blob-url');
  ...
  it('should not overwrite existing url on loaderConfig', async () => {
    ...
    const rep = makeRepresentation({
      file,
      url: 'https://example.com/existing-video.mp4',
      fps: 24,
    });
    const loader = new VideoRepresentationLoader();
    ...
    expect(rep.loaderConfig.url).toBe('https://example.com/existing-video.mp4');
  ...
  it('should load from url when file is absent', async () => {
    const rep = makeRepresentation({
      url: 'blob:http://localhost/mock-video-blob-url',
      path: 'video.mp4',
      fps: 24,
    });
    const loader = new VideoRepresentationLoader();
  ...
  it('should use default fps of 24 when not specified', async () => {
    const rep = makeRepresentation({
  ...
  it('should serialize video representation with url after load', async () => {
    const file = new File(['data'], 'video.mp4', { type: 'video/mp4' });
    const rep = makeRepresentation({ file, path: 'video.mp4', fps: 24 });
    const loader = new VideoRepresentationLoader();
    ...
    // url should survive
    expect(serialized.loaderConfig.url).toBe('blob:http://localhost/mock-video-blob-url');
    expect(serialized.loaderConfig.path).toBe('video.mp4');
  ...
  it('should round-trip: serialize then deserialize preserves url', async () => {
    ...
    expect(deserialized.loaderConfig.url).toBe('blob:http://localhost/mock-video-blob-url');
    expect(deserialized.loaderConfig.path).toBe('video.mp4');
  ...
  it('should round-trip: serialize, deserialize, then load from url', async () => {
    ...
    const file = new File(['data'], 'video.mp4', { type: 'video/mp4' });
    const originalRep = makeRepresentation({ file, path: 'video.mp4', fps: 24 });

    // 2. Load to populate url
    const loader1 = new VideoRepresentationLoader();
    ...
    const loader2 = new VideoRepresentationLoader();
  ...
  it('should work with proxy kind representations', async () => {
    const file = new File(['data'], 'proxy.mp4', { type: 'video/mp4' });
    const rep: MediaRepresentation = {
      ...makeRepresentation({ file, fps: 24 }),
      kind: 'proxy',
      priority: 2,
    };
    const loader = new VideoRepresentationLoader();
