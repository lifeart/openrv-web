...
const PREFACE_UL = [0x06, 0x0e, 0x2b, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01, 0x01, 0x01, 0x2f, 0x00];
...
const SEQUENCE_UL = [0x06, 0x0e, 0x2b, 0x34, 0x02, 0x53, 0x01, 0x01, 0x0d, 0x01, 0x01, 0x01, 0x01, 0x01, 0x0f, 0x00];

/** OP1a UL */
const OP1A_UL = [0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x01, 0x00, 0x00];
...
const ESSENCE_CONTAINER_AVC = [
  0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x0a, 0x0d, 0x01, 0x03, 0x01, 0x02, 0x10, 0x01, 0x00,
];

/** Essence Container UL for MPEG-2 */
const ESSENCE_CONTAINER_MPEG2 = [
  0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0d, 0x01, 0x03, 0x01, 0x02, 0x04, 0x01, 0x00,
];

/** Essence Container UL for MPEG-2 D10 */
const ESSENCE_CONTAINER_MPEG2_D10 = [
  0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0d, 0x01, 0x03, 0x01, 0x02, 0x02, 0x01, 0x00,
];
...
describe('MXF Demuxer', () => {
  ...
  describe('Detection', () => {
    ...
    it('MXF-DET-005: should reject buffer with correct prefix but wrong partition pack bytes', () => {
      ...
      const buffer = bytesToBuffer([
        0x06, 0x0e, 0x2b, 0x34, 0x01, 0x02, 0x03, 0x04, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x02,
      ]);
      expect(isMXFFile(buffer)).toBe(false);
    });
  });
  ...
  describe('KLV Parsing', () => {
    ...
    it('MXF-KLV-004: should extract correct key bytes', () => {
      const testUL = [0x06, 0x0e, 0x2b, 0x34, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c];
  ...
  describe('Metadata Extraction', () => {
    ...
    it('MXF-META-004: should extract timecode from Timecode Component set', () => {
      ...
      const trackValue = buildLocalSetValue([{ tag: 0x4801, value: buildRational(24, 1) }]);
      const trackKLV = buildKLV(TIMELINE_TRACK_UL, trackValue);
  ...
  describe('Error Handling', () => {
    ...
    it('MXF-ERR-002: should handle truncated partition pack gracefully', () => {
      ...
      const bytes = [0x06, 0x0e, 0x2b, 0x34, 0x02, 0x05, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x02];
      const buffer = bytesToBuffer(bytes);
      const metadata = parseMXFHeader(buffer);
  ...
  describe('Edge Cases', () => {
    ...
    it('should detect OPAtom operational pattern', () => {
      const opAtomUL = [0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x01, 0x0d, 0x01, 0x02, 0x01, 0x10, 0x01, 0x00, 0x00];
  ...
  describe('OP UL Version Byte', () => {
    it('should detect OP1a with different version byte', () => {
      ...
      const opULv2 = [0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x02, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x01, 0x00, 0x00];
    ...
    it('should detect OP1a with version byte 0x0d', () => {
      const opULv13 = [0x06, 0x0e, 0x2b, 0x34, 0x04, 0x01, 0x01, 0x0d, 0x0d, 0x01, 0x02, 0x01, 0x01, 0x01, 0x00, 0x00];
