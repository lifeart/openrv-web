...
const ESSENCE_ELEMENT_UL_PREFIX = [0x06, 0x0e, 0x2b, 0x34, 0x01, 0x02, 0x01, 0x01, 0x0d, 0x01, 0x03, 0x01] as const;
  ...
  if (firstByte < 0x80) {
    ...
    if (numBytes > 8) {
      throw new DecoderError(
        'MXF',
        `Invalid BER length encoding at offset ${berStart}: ${numBytes} length bytes exceeds maximum of 8`,
      );
    }
 ...
 */
export function parsePartitionPack(view: DataView, klvOffset: number, klv: KLVTriplet): MXFPartition {
...
function parseAudioDescriptor(view: DataView, valueOffset: number, valueLength: number): MXFEssenceDescriptor {
  const descriptor: MXFEssenceDescriptor = {
    type: 'audio',
    codec: 'pcm',
  };
      ...
      case TAG_AUDIO_SAMPLING_RATE:
        ...
        if (descriptor.sampleRate) {
          descriptor.audioSampleRate =
            descriptor.sampleRate.den !== 0 ? descriptor.sampleRate.num / descriptor.sampleRate.den : 0;
        }
        break;
