...
export function parseGainmapAVIF(buffer: ArrayBuffer): AVIFGainmapInfo | null {
  ...
  const metaResult = extractGainMapMetadata(view, buffer, items, locations, ipco.dataStart, ipco.dataEnd);
...
/**
 * Extract full gain map metadata from XMP or tmap box.
 * Returns both the headroom scalar and the full metadata.
 */
function extractGainMapMetadata(
  view: DataView,
  buffer: ArrayBuffer,
  items: ItemEntry[],
  locations: ItemLocation[],
