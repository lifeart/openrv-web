...
export function buildHueRotationMatrixMul(degrees: number): Float32Array {
  ...
  const T: Mat3 = [1 + dR, dG, dB, dR, 1 + dG, dB, dR, dG, 1 + dB];

  const TInv: Mat3 = [1 - dR, -dG, -dB, -dR, 1 - dG, -dB, -dR, -dG, 1 - dB];
 ...
 */
export function applyHueRotation(r: number, g: number, b: number, degrees: number): [number, number, number] {
