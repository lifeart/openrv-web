...
function createMockState(overrides: Partial<InternalShaderState> = {}): InternalShaderState {
  return {
    ...
    toneMappingState: {
      operator: 'off',
      reinhardWhitePoint: 1,
      filmicShoulderStrength: 0.15,
      filmicLinearStrength: 0.5,
      filmicLinearAngle: 0.1,
      filmicToeStrength: 0.2,
      filmicToeNumerator: 0.02,
      filmicToeDenominator: 0.3,
    ...
    falseColorLUTDirty: false,
    zebraEnabled: false,
    zebraHighThreshold: 0.95,
    zebraLowThreshold: 0.05,
    ...
    lut3DDirty: false,
    lut3DData: null,
    lookLUT3DDomainMin: [0, 0, 0] as [number, number, number],
    lookLUT3DDomainMax: [1, 1, 1] as [number, number, number],
    fileLUT3DEnabled: false,
    fileLUT3DIntensity: 1,
    fileLUT3DSize: 0,
    fileLUT3DDirty: false,
    fileLUT3DData: null,
    fileLUT3DDomainMin: [0, 0, 0] as [number, number, number],
    fileLUT3DDomainMax: [1, 1, 1] as [number, number, number],
    displayLUT3DEnabled: false,
    displayLUT3DIntensity: 1,
    displayLUT3DSize: 0,
    displayLUT3DDirty: false,
    displayLUT3DData: null,
    displayLUT3DDomainMin: [0, 0, 0] as [number, number, number],
    displayLUT3DDomainMax: [1, 1, 1] as [number, number, number],
    displayTransferCode: 1,
    displayGammaOverride: 1,
    displayBrightnessMultiplier: 1,
...
describe('ShaderPipeline', () => {
  ...
  it('A-9: N active stages = N draw calls with FBO alternation', () => {
    ...
    const allBindCalls = vi.mocked(gl.bindFramebuffer).mock.calls.filter((c) => c[0] === gl.FRAMEBUFFER);
  ...
  it('A-10: monolithic fallback when FBO allocation fails', () => {
    ...
    expect(fallbackFn).toHaveBeenCalledWith(gl, sourceTexture, state, texCb, null);
  });
  ...
  it('A-12: Global Uniforms UBO buffer created and bound with correct data', () => {
    ...
    expect(gl.bufferSubData).toHaveBeenCalledWith(gl.UNIFORM_BUFFER, 0, expect.any(Float32Array));
  ...
  it('A-17: passthrough vertex shader does not apply transforms', () => {
    ...
    const vertexSources = shaderSourceCalls.map((c) => c[1] as string).filter((s) => s.includes('a_position'));
  ...
  it('spatial stages use LINEAR filtering, non-spatial use NEAREST', () => {
    ...
    const linearCalls = texParamCalls.filter(
      (c) => c[1] === (gl as any).TEXTURE_MIN_FILTER && c[2] === (gl as any).LINEAR,
    );
  ...
  it('intermediate stages write to FBOs, not to screen', () => {
    ...
    const bindCalls = vi.mocked(gl.bindFramebuffer).mock.calls.filter((c) => c[0] === gl.FRAMEBUFFER);
  ...
  it('execute throws when stage shader fails to compile', () => {
    ...
    expect(() => pipeline.execute(gl, sourceTexture, 1920, 1080, state, texCb)).toThrow('Shader compile error');
  });
  ...
  it('execute throws when stage shader fails to link', () => {
    ...
    expect(() => pipeline.execute(gl, sourceTexture, 1920, 1080, state, texCb)).toThrow('Shader program link error');
  });
